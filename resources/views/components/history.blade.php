    ...
    <ul {{ $attributes->merge(['class' => 'list-group list-group-flush history']) }} >
      @foreach ($items as $key => $item)
        <li class="list-group-item d-flex">
          <span class="history-tooltip p-2 mr-2 badge bg-{{ $color($item->event) }}" data-bs-toggle="tooltip" data-placement="top" title="@lang('trackable.event.'.$item->event)">
            <i class="icon-{{ $icon($item->event) }}" aria-label="@lang('trackable.event.'.$item->event)"></i>
          </span>
          @if($item->relation)
            <span class="history-tooltip p-2 mr-2 badge bg-secondary" data-bs-toggle="tooltip" data-placement="top" title="@choice('model.'.$item->relation,1) {{ $item->comment ? ': '.$item->comment :'' }}" >

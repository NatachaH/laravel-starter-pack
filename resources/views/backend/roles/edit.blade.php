...
@section('content')
  ...
  <form action="{{ route('backend.roles.update', $role->id) }}" method="POST" autocomplete="off">
      ...
      <x-ac-permission-fieldset :legend="trans_choice('backend.model.permission',2)" :checked="$role->permissions->modelKeys()" translation="backend.model" :disabled="$permissionsDisabled"/>

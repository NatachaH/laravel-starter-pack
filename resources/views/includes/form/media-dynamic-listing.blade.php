  ...
  @if($hasPreview || $hasDownload)
    <x-slot name="after">
        @if($hasPreview && $item->thumbnail)
            <button type="button" class="btn btn-gray popover-media" data-container="body" data-toggle="popover" data-content="<img src='{{ $item->thumbnail }}' />" aria-label="@lang('sp::action.preview')">
